 ...
 * каждый раз, когда сумма покупок продукта за минуту было больше, чем MAX_PURCHASES_QUANTITY_PER_MINUTE
 */
public class DZ {
    ...
    private static final long MAX_PURCHASES_QUANTITY_PER_MINUTE = 3000L;
    ...
    public static Topology buildTopology(SchemaRegistryClient client, Map<String, String> serDeConfig) {
        ...
        purchasesStream.groupBy((key, val) -> val.get("productid").toString(), Grouped.with(new Serdes.StringSerde(), avroSerde))
                ...
                .filter((key, val) -> val > MAX_PURCHASES_QUANTITY_PER_MINUTE)
                .toStream()
                .map((key, val) -> {
